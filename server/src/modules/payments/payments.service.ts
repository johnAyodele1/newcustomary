...
import type { InitializePaymentInput } from './payments.schemas.js';

type PaystackResponse = { status: boolean; message: string; data?: { authorization_url: string; access_code: string; reference: string } };
...
export async function initializePayment(input: InitializePaymentInput) {
  ...
  const response = await fetch('https://api.paystack.co/transaction/initialize', {
    ...
    body: JSON.stringify({ email: input.email, amount: input.amount * 100, reference: input.reference, currency: 'NGN' }),
  });
